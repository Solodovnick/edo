...
**1.3.2:** Добавлен `docs/api-cabinets.md` — маппинг REST `/api/v1` на кабинеты (Регистрация / Процесс / Руководитель / Аудит), UC и словарь данных; расширена роль `.cursor/rules/role-analyst-requirements.mdc` и навык `edo-ui-analyst`; UI кабинетов переведён на вызовы API через `src/services/edoCabinetApi.ts` вместо статических массивов `unifiedAppealsData` / `auditMockData`.
